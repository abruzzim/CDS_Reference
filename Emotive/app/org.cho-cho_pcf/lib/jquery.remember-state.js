  ...
  */
  ...
  var remember_state = {
    ...
    clickNotice: function(e) {
      var data = JSON.parse(localStorage.getItem(e.data.instance.objName)).formData,
          $f = $(this).closest("form"),
          $e;
      for (var i in data) {
    ...
    createNoticeDialog: function() {
      ...
      if (localStorage[this.objName]) {
          storedObj = JSON.parse(localStorage[this.objName]);
          if (storedObj.username && (storedObj.username === Emotive.User.getName()))
          {
              if (this.noticeDialog.length && this.noticeDialog.jquery) {
  ...
  var internals = {
    setObject: function(key, value) { localStorage[key] = JSON.stringify({formData: value, username:Emotive.User.getName()}); },
